...
function _tickSpawns(roundNum) {
  _pendingSpawns.forEach((s, idx) => {
    ...
    if (s.stationary)                     u.stationary = true;   // never moves in combat (holds position)
...
function _spawnZoneEnemy(e) {
  ...
  if (e.y != null)                      { u.grp.position.y = e.y; u._level = e.y; }
  ...
  if (e.stationary)           u.stationary = true;   // never moves in combat (holds position)
