...
// for every unit every frame (main.js), 12x per line-of-sight test (LOS_STEPS), and 3x per
// engaged pair per frame. It used to
...
// This is the surface of the uncarved landform (e.g. the top of a hill), used by
// getTerrainHeight to blend a trench's rising walls back into the surrounding rock.
...
// ── "Ground" height ─────────────────────────────────────────────────────────────
// The walkable surface height at (x,z). Kept as a thin alias over getTerrainHeight so
// the many callers that still pass a now-ignored third `layer` argument keep working
// unchanged after the cave surface/under LAYER system was removed.
export function getGroundHeight(x, z) { return getTerrainHeight(x, z); }
